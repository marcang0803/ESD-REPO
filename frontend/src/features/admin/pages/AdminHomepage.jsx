...
export default function AdminHomepage() {
  ...
  useEffect(() => {
    fetchClasses(PROVIDER_ID)
      .then(list => {
        setClasses(list.filter(c => c.status === 'Scheduled'))
  ...
  function closeModal() {
    ...
    setPayoutResult(null)
    // Refresh classes so newly completed ones move to Past Classes
    fetchClasses(PROVIDER_ID)
      .then(list => {
        setClasses(list.filter(c => c.status === 'Scheduled'))
        setCompletedClasses(list.filter(c => c.status === 'Completed'))
  ...
  return (
    <div style={{ backgroundColor: colors.cream, minHeight: '100vh', paddingBottom: 120 }}>
      ...
      <main style={{ padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: 48 }}>
        ...
        {/* ── Past Classes (Scenario 3 demo) ────────────── */}
        {completedClasses.length > 0 && (
          <section style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontFamily: fonts.serif, fontWeight: 700, fontSize: 22, color: colors.textDark, margin: 0 }}>Past Classes</h2>
              <span style={{ fontFamily: fonts.sans, fontSize: 12, color: colors.textMuted, background: colors.cardBg, borderRadius: radius.pill, padding: '4px 12px' }}>
                Payout pending
              </span>
            </div>
            <p style={{ fontFamily: fonts.sans, fontSize: 13, color: colors.textMuted, margin: 0, lineHeight: 1.5 }}>
              These classes have concluded. Click <strong>Complete &amp; Pay Out</strong> to trigger the Stripe payout to the provider.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {completedClasses.map((cls) => {
                ...
                const booked = cls.capacity - cls.available_slots
                return (
                  <div key={cls.class_id} style={{ position: 'relative', opacity: 0.85 }}>
                    <ClassCard
                      month={month}
                      day={day}
                      title={cls.class_name}
                    ...
                    />
                    <button onClick={() => openPayoutModal(cls)}
                      style={{ position: 'absolute', bottom: 16, right: 52, backgroundColor: colors.brownDark, color: colors.white, fontFamily: fonts.sans, fontWeight: 600, fontSize: 11, border: 'none', borderRadius: radius.pill, padding: '6px 14px', cursor: 'pointer' }}>
                      Complete &amp; Pay Out
                    </button>
                  </div>
                )
