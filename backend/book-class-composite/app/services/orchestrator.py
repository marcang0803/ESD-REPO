    ...
    def create_booking(self, user_id, class_id, idempotency_key):
        ...
        try:
            ...
            if not booking_response.get("success"):
                # Check if this is an idempotent duplicate (user already booked this class)
                already_booked = (
                    "already booked" in booking_response.get("message", "").lower()
                    or booking_response.get("code") == 409
                )

                if already_booked:
                    # Treat as success — do NOT rollback the slot or wallet
                    print("Duplicate booking detected — idempotent, skipping rollback")
                    booking_data = booking_response.get("data")
                    if not booking_data:
                        return {
                            "success": False,
                            "message": "Booking already exists but could not retrieve booking data.",
                            "http_status": 409
                        }
                else:
